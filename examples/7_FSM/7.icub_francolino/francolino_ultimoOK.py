...
class DynamicFSMServer(iCubRESTApp):
    ...
    def _register_action_endpoints(self):
        ...
        try:
            ...
            self.rest_manager._flaskapp_.add_url_rule(
                f"{prefix}/actions/<action_name>",
                'delete_action_route',
                lambda action_name: self.delete_action(action_name=action_name),
                methods=['DELETE']
            )
            self.logger.info(f"  - Registrato endpoint: DELETE {prefix}/actions/<action_name>")
    ...
    def create_action(self, **action_data):
        ...
        try:
            ...
            if not action_name:
                return {"status": "error", "message": "La chiave 'name' è obbligatoria nel JSON dell'azione."}, 400
